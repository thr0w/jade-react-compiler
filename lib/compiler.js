...
Compiler.prototype = {
  ...
  compile: function () {
    ...
    this.scope = this.newscope(false, true)
    var ast = b.program([this.generate()])
    asttypes.defineMethod('at', at)
  ...
  this.createElement=function(scope, name, props, children){
    scope.element_count++;

    var callee = b.memberExpression(b.identifier('React'), b.identifier('createElement'), false).at(tag)
    var args = [name]
  ...
  visitCase: function (node) {
    ...
    this.scope.needs_function = true;
    var cases = this.visit(node.block)
    return b.switchStatement(discriminant, cases).at(_loc)
  ...
  visitWhen: function (node, start) {
    ...
    this.scope.after_push.push(function(){
  ...
  visitBlock: function (block) {
    var scope = this.scope
    return block.nodes.map(function(node){
      this.scope = scope
      var chield = _this.visit(node)

    }.bind(this))
    var _this = this, ret
    var in_scope = this.scope
    var new_scopes = false, $ret_push
    ...
    if ($ret && !in_scope:root) {
      ...
      ret = []
      in_scope:children = save_in_scope:children
    ...
    function stmt_ret (stmt) {
      ...
      else if (n.ExpressionStatement.check(stmt) && isReactCreateElement(stmt.expression))
  ...
  visitMixin: function (mixin) {
    assert(mixin.attributeBlocks.length == 0)
    assert(mixin.attributeNames.length == 0)
    assert(mixin.attrs.length == 0)
    var $ret = b.identifier('$ret').at(mixin)
    if (mixin.call) { // Use
      var args
      if (mixin.args) {
        args = parseJs('[' + mixin.args + ']', false, true, loc(mixin)).elements
      }
      else args = []
      args.unshift($ret)
      if (mixin.block) {
        this.in_scope:children = true
        var mixin_block = this.visit(mixin.block)
        args.push(b.arrayExpression(mixin_block).at(mixin))
      }
      return {
        $ret: true,
        stmt: b.expressionStatement(b.callExpression(b.identifier(mixin.name).at(mixin), args).at(mixin)).at(mixin)
      }
    } else { // declare
      assert(mixin.block != null)
      var mixinCompiler = new Compiler(null,
        mixin.block,
        {sourceFileName: mixin.name, raw: true})

      mixinCompiler.in_scope:root = false
      mixinCompiler.in_scope:children = 'push'

      var fn = mixinCompiler.generate()
      assert(fn.params.length == 0)
      if (mixin.args) {
        fn.params = mixin.args.split(',').map(function (arg) {
          return b.identifier(arg).at(mixin)
        })
      }
      fn.params.unshift($ret)
      fn.params.push(b.identifier('$block').at(mixin))
      return fn
    }
  },
  /**
  ...
  visitMixinBlock: function (block) {
    return b.identifier('$block').at(block)
  },
  ...
  visitTag: function (tag) {
    ...
    var children = (tag.block.nodes.length ? tag.block.nodes : [])
      .map(function(chield){
        scope = this.newscope();
        return this.visit(chield)
      }.bind(this));
    if (tag.code) {
    ...
    return my_scope.create_element(name, attrs, children)
  },
  ...
  visitScript: function (tag) {
    ...
    js = js.filter(function (stmt) {
      if (n.ImportDeclaration.check(stmt)) {
        _this.imports.push(stmt)
        return false
      }
      return true
    })
  ...
  visitCode: function (code) {
    ...
    if (js.length > 0 && n.VariableDeclaration.check(js[0]) && !js[0].declarations[0].init) {
      js[0].declarations.forEach(function (v) {
        if (!v.id.typeAnnotation)
          this.error('Argument needs type annotation', code)
        if (v.init)
          this.error("Argument can't be initialized")
...
function isReactCreateElement (node) {
  ...
  n.Identifier.check(node.callee.object) && node.callee.object.name === 'React' &&
  n.Identifier.check(node.callee.property) && node.callee.property.name === 'createElement'
}
