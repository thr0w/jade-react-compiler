...
Compiler.prototype = {
  ...
  generate: function () {
    ...
    this.fn_arguments = []
    this.depth = -1
    var body = this.function_body(this.node)
  ...
  function_body: function(node){
    var block = this.visit(node)
    assert(Array.isArray(block))
    var body = [], ret
  ...
  visitBlock: function (block, start) {
    return block.nodes.map(this.visit)
  },
  ...
  visitEach: function (each, start) {
    ...
    var body = this.function_body(each.block);
