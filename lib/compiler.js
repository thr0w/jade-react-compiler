...
Compiler.gen_annotation = gen_annotation

function gen_annotation (annotation) {
  var s
  switch (annotation.type) {
    case 'TypeAnnotation':
      s = gen_annotation(annotation.typeAnnotation)
      break
    case 'Identifier':
      s = escodegen.generate(annotation)
      break
    ...
    case 'GenericTypeAnnotation':
      s = gen_annotation(annotation.id)
      break
    case 'QualifiedTypeIdentifier':
      s = gen_annotation(annotation.qualification) + '.' + gen_annotation(annotation.id)
      break
    default:
      throw 'annotation não suportado: ' + JSON.stringify(annotation)
  }
  if (annotation.type != 'TypeAnnotation' && annotation.typeAnnotation) {
    throw 'TODO'
  }
  return s
}
