...
function compileComponent (src, options) {
  ...
  this_was_replaced = false
  render_fn = apply_this(render_fn)
  if (this_was_replaced)
  ...
  raw.ast.body = raw.imports.concat(requires).concat([createClass_var]).concat(export_class)
  ...
  function visit_require (match) {
    return {
      parse: match[0],
      fn: function ( stmts) {
        requires = requires.concat(stmts)
      }
    }
  }
